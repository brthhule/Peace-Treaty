...
void synopsis()
{
    ...
    std::cout << "- Although players cannot save their complete data, a portion of the land that they gained in the previous game (as well as the amount of resources available) will be available through a code " << std::endl << std::endl;
}
...
void casualtyReport(int troopsLost[5], int injuredTroops[5])
{
    ...
    for (int x = 0; x < 5; x++) /*print out deaths*/
    {
        std::cout << troopNames[x] << " injured: " << injuredTroops[x] << std::endl;
    }
    std::cout << std::endl;
}
